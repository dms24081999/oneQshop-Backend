...
def get_token_generator():
    ...
    return token_class(**options)  # initialize the token class and pass options
...
class RandomStringTokenGenerator(BaseTokenGenerator):
    ...
    def generate_token(self, *args, **kwargs):
        ...
        return binascii.hexlify(os.urandom(self.max_length)).decode()[
            0:length
...
class RandomNumberTokenGenerator(BaseTokenGenerator):
    ...
    def generate_token(self, *args, **kwargs):
        ...
        return str(
            r.randint(self.min_number, self.max_number)
